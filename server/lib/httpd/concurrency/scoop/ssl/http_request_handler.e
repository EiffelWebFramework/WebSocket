...
feature -- Change
	...
	set_client_socket (a_socket: separate HTTP_STREAM_SOCKET)
		local
			retried: BOOLEAN
			l_client_socket: HTTP_STREAM_SSL_SOCKET
		do
			if retried then
				has_error := True
			else
				if attached {separate HTTP_STREAM_SSL_SOCKET} a_socket as l_socket  then
					create l_client_socket.make_from_separate (l_socket)
					client_socket := l_client_socket
					client_socket_source := l_socket
				else
